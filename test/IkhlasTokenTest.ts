...
let signers: any;
  ...
  beforeEach(async function () {
   ...
   it("Should return the defined symbol, name and decimals for the token", async function () {
    ...
    const ikhlasToken = await new IkhlasToken__factory(owner[0]).deploy();
  ...
  it("Should not allow non-owner to run the setStakingContract function", async () => {
    ...
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
  ...
  it("Should allow owner to run the setStakingContract function", async () => {
    const [owner, second, third] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await ikhlasToken.connect(owner).setStakingContract("0x0000000000000000000000000000000000000000");
  ...
  it("Should return the correct balanceof with the balanceof function", async () =>  {
    const owner = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner[0]).deploy();
    expect(await ikhlasToken.balanceOf(owner[0].address)).to.be.equal(ethers.utils.parseUnits("1", 24));
  });

  it("Should increase the approve with the approve function", async () => {
    const [owner, second] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    expect(await ikhlasToken.connect(owner).approve(second.address, (ethers.utils.parseUnits("1", 20)))).to.emit(ikhlasToken, "Approval").withArgs(owner.address, second.address, (ethers.utils.parseUnits("1", 20)));
  });
  ...
  it("Should check the balance of msg.sender about availability of tokens greater than value prior to executing the approve function", async () => {
    const [owner, second] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await expect(ikhlasToken.connect(owner).approve(second.address, (ethers.utils.parseUnits("2", 24)))).to.be.revertedWith("msg.sender does not have sufficient tokens");
  });

  it("Should check if msg.sender token balance is greater than the value prior to executing the transfer function", async () => {
    const [owner, second] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await expect(ikhlasToken.connect(owner).transfer(second.address, (ethers.utils.parseUnits("2", 24)))).to.be.revertedWith("msg.sender does not have sufficient tokens");
  });

  it("Should transfer the tokens with the transfer function", async () => {
    const [owner, second] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    expect(await ikhlasToken.connect(owner).transfer(second.address, (ethers.utils.parseUnits("1", 20)))).to.emit(ikhlasToken, "Transfer").withArgs(owner.address, second.address, (ethers.utils.parseUnits("1", 20)));
  });
  
  it("Should transfer the tokens with the transferfrom function", async () => {
    const [owner, second, third] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await ikhlasToken.connect(owner).approve(second.address, (ethers.utils.parseUnits("1", 20)));
    expect(await ikhlasToken.connect(second).transferFrom(owner.address, third.address, (ethers.utils.parseUnits("1", 20)))).to.emit(ikhlasToken, "Transfer").withArgs(owner.address, third.address, (ethers.utils.parseUnits("1", 20)));
  });
  ...
  it("Should check availability of tokens in message sender prior to executing the the tokens with the transferfrom function", async () => {
    ...
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await ikhlasToken.connect(owner).approve(second.address, (ethers.utils.parseUnits("1", 20)));
    await expect(ikhlasToken.connect(second).transferFrom(owner.address, third.address, (ethers.utils.parseUnits("2", 24)))).to.be.revertedWith("msg.sender does not have sufficient tokens");
  });
  ...
  it("Should check allowance balance of tokens prior to executing the transferfrom function", async () => {
    ...
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await ikhlasToken.connect(owner).approve(second.address, (ethers.utils.parseUnits("1", 20)));
    await expect(ikhlasToken.connect(second).transferFrom(owner.address, third.address, (ethers.utils.parseUnits("2", 21)))).to.be.revertedWith("value exceeding the allowance limit");
  });
  ...
  it("Should display the allowance with the Allowance function", async () => {
    ...
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await ikhlasToken.connect(owner).approve(second.address, (ethers.utils.parseUnits("1", 20)));
    expect(await ikhlasToken.connect(second).allowance(owner.address, second.address)).to.be.equal(ethers.utils.parseUnits("1", 20));
  });
  ...
  it("Should allow owner to run the mint function", async () => {
    const [owner, second, third] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    expect (await ikhlasToken.connect(owner).mint(second.address, (ethers.utils.parseUnits("1", 20)))).to.emit(ikhlasToken, "Transfer").withArgs('0x0000000000000000000000000000000000000000', second.address, (ethers.utils.parseUnits("1", 20)));
  });
  ...
  it("Should check if address from which funds will be burned has sufficent tokens prior to running the burn function", async () => {
    const [owner, second, third] = await ethers.getSigners();
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await ikhlasToken.connect(owner).transfer(second.address, (ethers.utils.parseUnits("1", 20)));
    await expect (ikhlasToken.connect(owner).burn(second.address, (ethers.utils.parseUnits("2", 20)))).to.be.revertedWith("value to be burned exceeds the balance");
  });
  ...
  it("Should allow owner to run the burn function", async () => {
    ...
    const ikhlasToken = await new IkhlasToken__factory(owner).deploy();
    await ikhlasToken.connect(owner).transfer(second.address, (ethers.utils.parseUnits("1", 20)));
    expect (await ikhlasToken.connect(owner).burn(second.address, (ethers.utils.parseUnits("1", 20)))).to.emit(ikhlasToken, "Transfer").withArgs(second.address, '0x0000000000000000000000000000000000000000', (ethers.utils.parseUnits("1", 20)));
  });
